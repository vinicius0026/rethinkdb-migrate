...
module.exports = function (opt) {
  emitter.emit('info', 'Validating options')
  return validateOptions(opt)
    .then(options => {
      emitter.emit('info', 'Connecting to Rethinkdb')
      return options
    })
    .then(connectToRethink)
    .then(options => {
      emitter.emit('info', 'Creating DB if inexistent')
      return options
    })
    .then(createDbIfInexistent)
    .then(options => {
      emitter.emit('info', 'Executing Migration')
      return options
    })
    .then(executeMigration)
    .then(options => {
      emitter.emit('info', 'Closing Connection')
      return options
    })
