...
const Migrate = function (opt) {
  ...
  return validateOptions(opt)
    ...
    .then(emit('info', 'Creating DB if inexistent'))
    .then(createDbIfInexistent)
    .then(emit('info', 'Executing Migration'))
...
function migrateUp (options) {
  emit('info', 'Getting latest executed')()
  return getLatestMigrationExecuted(options)
    .then(emit('info', 'Getting unexecuted migrations'))
    .then(latest => getUnExecutedMigrations(latest, options))
    .then(emit('info', 'Running migrations'))
    ...
    .then(emit('info', 'The end'))
    .then(() => options)
}

function migrateDown (options) {
  emit('info', 'Getting all executed migrations')()
  return getAllMigrationsExecuted(options)
    .then(emit('info', 'Loading Migrations Code'))
    .then(migrations => loadMigrationsCode(migrations, options))
    .then(emit('info', 'Undoing migrations'))
